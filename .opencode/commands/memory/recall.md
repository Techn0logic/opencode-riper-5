...
✅ **Correct**: `[ROOT]/.opencode/memory-bank/main/session.md` (where ROOT = `git rev-parse --show-toplevel`)
❌ **Wrong**: `[ROOT]/packages/react/.opencode/memory-bank/main/session.md`
❌ **Wrong**: `packages/react/.opencode/memory-bank/main/session.md`
...
```bash
last_memory=$(ls -t $(git rev-parse --show-toplevel)/.opencode/memory-bank/$(git branch --show-current)/sessions/*.md 2>/dev/null | head -1)
...
```bash
git log -n 5 -p -- .opencode/memory-bank/
```

## Search Scope
Looking for memories in: `[ROOT]/.opencode/memory-bank/!`git branch --show-current`/` (where ROOT = `git rev-parse --show-toplevel`)

## Search Query
$ARGUMENTS

## Available Memories
!`ls -la $(git rev-parse --show-toplevel)/.opencode/memory-bank/$(git branch --show-current)/ 2>/dev/null || echo "No memories found for current branch"`
